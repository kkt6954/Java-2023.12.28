...
public class Object01 {
	...
	public static void main(String[] args) {
		...
		//		볼보 xc60 흰색 자동차 등 > 클래스를 토대로 실제로 사용할 수 있는 객체를 만듬
		
		// 3. 상속(inheritance)
		// : 한 클래스(부모)의 속성과 메소드를 다른 클래스(자식)가 받아서 사용하는 기능
		// ex) '전기 자동차' 클래스
		//		- 일반 자동차 클래스의 모든 특성을 상속받고(43행의 ex에 있는 자동차 클래스가 부모)
		//		- 추가적으로 배터리 관련 속성과 충전 메소드를 가짐
		
		// 4. 다형성(Polymorphism)
		// : 같은 메소드 호출이 다른 클래스의 객체에 대해 다른 방식으로 실행가능
		// ex) '주행'이라는 메소드(행동)는 '전기 자동차'객체에서는 전기를 사용
		//								'가솔린자동차'객체에서는 가솔린을 사용하여 주행
		
		// 5. 캡슐화(Encapsulation)
		// : 객체의 데이터를 외부로부터 숨기고, 내부의 메소드를 통해서만 해당 데이터를 조작할 수 있게 하는 것
		// ex) 자동차의 엔진은 외부에서 직접 조작 불가
		//		엔진을 조작하기 위해서는 시동 버튼, 가속 페달 등을 사용
		
		// 6. 추상화(Abstraction)
		// : 복잡한 세부사항을 숨기고 사용자에게 필요한 정보만을 제공
		// ex) 자동차 운전자는 자동차의 모든 부품, 원리를 알지 않아도 방향, 속도, 정지 등의 기본 기능만 이해하면 운전 가능
		...
		// 자바(JAVA)에서의 객체 지향 프로그램(자바에서는 순수하게 객체 지향 프로그램만 사용)
