...
// 정적 메서드
// : 메서드 앞에 static 키워드를 붙이는 경우 클래스(정적)에서도 생성가능
...
// static 사용 권장 사항
// 1. 클래스 설계시, 변수(속성) 중 모든 인스턴스가 공통적으로 사용하는 값
// 2. 클래스 변수는 인스턴스 생성없이도 사용 가능
// 3. 클래스 메서드는 인스턴스 변수를 사용할 수 없다.
// - 클래스 메서드 내에서 인스턴스 변수 호출(사용) X
// 4. 메서드 내에서 인스턴스 변수를 사용하지 X, static를 붙이는 것을 고려
...
class Operator {
	...
	static int add( int a, int b ) { return a + b; } // a, b는 지역 변수
