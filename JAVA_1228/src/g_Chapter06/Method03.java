...
// : add, subtract, multply, divide의 4개 메서드
class MyMath {
	int add(int a, int b){ return a + b;}
	int subtrcat(int a, int b){ return a - b;}
	...
	// : - 자바 컴파일러가 메서드의 마지막에 자동으로 return;을 추가(생성자를 명시하지 않아도 자동으로 생성자를 명시)
	...
	int max(int a, int b) {
		if (a > b) {
			// a가 b보다 클 경우에만 반환하기 때문에 return문이 없을 수도 있다는 오류
			// 조건이 참일때만 실행
			return a;
		} else {
			// 항상 결과값이 반환되도록 지정
	...
	// 반환값
	// : 주로 계산한 결과를 변수에 저장하여 변수 반환를 반환하는 경우가 많음 
	// - return values,
	// : 수식 자체를 반환하는 것도 가능
	// - return x + y;
	...
	// : 조건에 따라 코드  실행 여부를 제어
//	int min(int a, int b) {
//		// if - else 조건문의 경우 실행할 코드 블럭 한줄이면 중괄호 생략
//		if (a > b)return b;
//		else return a;
...
public class Method03 {
	...
	public static void main(String[] args) {
		...
		// 		, 반한된 값을 value 변수에 할당
		//		, 함수가 void 타입으로 바로 출력하는 경우 console창에 출려
		int value1 = mm.add(10, 20); // 메서드 호출
		int value2 = mm.subtrcat(20, 10);
		int value3 = mm.multply(5, 3);
