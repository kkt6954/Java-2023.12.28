...
// 매개변수 선언(Parameter)
// : 메서드가 작업을 수행하는데 필요한 값(입력)을 제공받기 위한 것
// : 필요한 값의 개수만큼 변수를 선언하여 각 변수 간의 구분은 쉼표(,)를 사용
// : 변수들의 데이터 타입이 같더라도 변수의 타입을 생략할 수 없음
// int add (int x, y) {...} - Error

// 반환 타입(return type)
// : 메서드의 작업수행 결과(출력)인 반환 값의 타입을 명시
...
// : 메서드의 구현 부에 작성된 변수뿐만 아니라 매개변수도 메서드 내에서 선언된 것으로 간주하기 때문에 지역변수로 인식


class Example{
	// 인스턴스 변수 선언
	// 데이터 타입이 같은 경우 변수타입 생략이 가능
//	int x;
//	int y;
	int x, y;
	...
	// 메서드 내에 선언된 지역 변수는 메서드 내에서만 사용할 수 있기 떄문에
...
public class Method02 {
	// 클래스 파일
	// main method
	// : 접근제어자 정적메소드 반환타입 메서드명(매개변수){구현부}
	...
	public static void main(String[] args) {
		...
		// 인수(인자)
		// : 메서드를 호출할때 소괄호()안에 지정해준 값
		// : 인자의 개수와 순서는 반드시 호출된 메서드에 선언된 매개변수와 일치
//		ex1.add(10, 20, 30); - Error // 메서드에 선언된 매개변수와 개수가 달라서 오류
//		ex1.add(10.0, 20.0); - Error // double 타입은 int 타입에 사용 불가
		ex1.add('a', 'b'); // char 타입은 int 타입으로 자동형변환이 가능하므로 오류X
