...
public class D_Array04 {
	...
	public static void main(String[] args) {
		...
		// 다차원(주로 2차원) 배열의 선언 및 초기화
		// 2차원 배열 선언
		// : 데이터타입[][] 배열명; (3차원일 경우 데이터타입[][][] 배열명;)
		int[][] matrix;
		// 2차원 배열 초기화 : 배열명 = new int[행수][열수];
		
		// 다차원 배열의 경우 각 행에서 열의 개수가 서로 달라도 가능
		// 배열을 선언할때 열의 크기는 지정하지 않아도 가능
		// , 행의 크기는 반드시 지정
		// 배열명 = new int[필수][선택];ㅣ
		matrix = new int [3][3];
...
//		matrix 배열과 array의 구조가 같다.
		...
		for(int i = 0; i < length; i++) {
			for(int j = 0; j < innerLength; j++) {
				System.out.print(threeFour[i][j]); // 행안의 요소를 가로로 나열(println이 아닌 print일 경우)
			}
