...
// 자식 클래스의 객체를 생성할때 자식 클래스는 자식 생성자를 통해 자식 객체 생성
...
class Cat extends Mammal {
	...
	// , super() 부모클래스 생성자 호출 키워드가 정의되어 있음
	
	// 2. super 참조 변수
	void greet() {
		// 해당 클래스가 가진 인스턴스 변수에 접근(생략가능)
		System.out.println("Hello" + " " +  this.name);
		// 부모 클래스가 가진 인스턴스 변수에 접근(super 키워드 사용), 생략불가
...
public class Inheritance02 {
	public static void main(String[] args) {
		...
		// : 단, 항상 직접적으로 정의된 생성자를 필요로 하는 것은 아님
		Cat voidCat = new Cat();
		Cat myCat = new Cat("choco");
							// *
		// 자식 클래스의 child 호출 후
		// 위의 인스턴스 생성시 부모 클래스로 전달했던 choco이름(*표시)의 부모 클래스의 name이 호출
		myCat.greet();
	}
