...
//					변수를 선언하면 행당되는 자료형의 크기만큼 메모리 할당
//		- 참조 타입 : 객체의 참조(메모리 주소)를 저장하는 변수 타입
		
//		2. 기본 자료형
//		: 총 8가지 기본형 타입이 존재(정수 타입,

//		2-1. 정수 타입(8bit == 1byte)
//		byte : 8bit 정수 - 값의 범위(-128 ~ 127)
//		short : 2byte 정수 - 값의 범위(-32,768 ~ 32,767)==(-|2의 15승| ~ |2의 15승|-1)
//		int(기본값) 4byte 정수 - 값의 범위(약-2.1억 ~ 약2.1억-1)
//		long : 8byte 정수 - 값의 범위(-2의63제곱 ~ 2의63제곱-1)
		int age;
		age = 27;
		System.out.println(age);
		
//		long 데이터 타입은 숫자 뒤에 알파벳 L 또는 l 을 써서 long 형임을 표시
		long apartmentPrice = 1000000000L;
		
//		2-2. 문자형
//		char(캐릭터) : 하나의 문자를 저장하기 위한 변수를 선언할 때 사용
//		- 문자형의 데이터의 경우 따옴표를 사용해 감싸야함
//		- 2byte
		char grade = 'A';
		char lastName = '이';
		
//		2-3. 실수형(부등소수점 타입) : 소수점 자리가 있는 수
//		float : 4byte 소수점, 숫자 뒤에 알파벳 F를 써서 float 형임을 표시
//		double(기본형) : 8byte 소수점, 기본 실수 타입
		float height = 169.2F;
		double price = 19.3;
