...
interface ExampleInterface {
	int EXAMPLE_VARIABLE = 10; // public, static, final 특성을 가짐
	void printVar();
	default void method() {
...
// 다중 인터페이스 사용 방법은 (,)로 구분하며 나열
// 주의사항
// : 두 인터페이스 같은 시그니처(같은 선언부)를 가진 디폴트 메서드가 존재한다면
// : 선언이 같고 구현이 다른 메서드가 충돌
// : >> 해당 클래스는 반드시 재정의해야 함
class ExampleClass implements ExampleInterface, ExampleInterface2 {
	public void printVar (){ System.out.println(EXAMPLE_VARIABLE); }
	public void printVar2 () { System.out.println("다중 인터페이스 구현"); }
	@Override
	public void method() {
...
// public : 모든 곳에서 접근가능
// static : 인스턴스화 없이 인터페이스명을 통해 접근이 가능
